...
import org.eulerframework.common.util.NullSafeUtils;
...
public final class ContentDisposition {
	...
	public boolean equals(@Nullable Object other) {
		return (this == other || (other instanceof ContentDisposition that &&
				NullSafeUtils.nullSafeEquals(this.type, that.type) &&
				NullSafeUtils.nullSafeEquals(this.name, that.name) &&
				NullSafeUtils.nullSafeEquals(this.filename, that.filename) &&
				NullSafeUtils.nullSafeEquals(this.charset, that.charset) &&
				NullSafeUtils.nullSafeEquals(this.size, that.size) &&
				NullSafeUtils.nullSafeEquals(this.creationDate, that.creationDate)&&
				NullSafeUtils.nullSafeEquals(this.modificationDate, that.modificationDate)&&
				NullSafeUtils.nullSafeEquals(this.readDate, that.readDate)));
	}

	@Override
	public int hashCode() {
		return NullSafeUtils.nullSafeHash(this.type, this.name,this.filename,
			...
			if (eqIndex != -1) {
				...
				if (attribute.equals("name") ) {
					...
					if (idx1 != -1 && idx2 != -1) {
						...
						AssertUtils.isTrue(UTF_8.equals(charset) || ISO_8859_1.equals(charset),
	...
	private static String decodeRfc5987Filename(String filename, Charset charset) {
		AssertUtils.notNull(filename, "'filename' must not be null");
	...
	private static String encodeRfc5987Filename(String input, Charset charset) {
		AssertUtils.notNull(input, "'input' must not be null");
		AssertUtils.notNull(charset, "'charset' must not be null");
		AssertUtils.isTrue(!StandardCharsets.US_ASCII.equals(charset), "ASCII does not require encoding");
		AssertUtils.isTrue(UTF_8.equals(charset) || ISO_8859_1.equals(charset), "Only UTF-8 and ISO-8859-1 are supported");
	...
	private static class BuilderImpl implements Builder {
		...
		public BuilderImpl(String type) {
			AssertUtils.hasText(type, "'type' must not be not empty");
			this.type = type;
		}
